package com.cordova.plugin.handleintentplugin;
 ...
 */
public class HandleIntentPlugin extends CordovaPlugin {

    private final String pluginName = "HandleIntentPlugin";
     ...
     * @param context
     */
    public boolean setResultAndFinishActivity (final JSONArray data, final CallbackContext context) throws JSONException {
        if(data.length() != 2) {
            context.sendPluginResult(new PluginResult(PluginResult.Status.INVALID_ACTION));
            return false;
        }

        Intent intentResult = cordova.getActivity().getIntent();
        intentResult.putExtra("message", data.getString(1));
        cordova.getActivity().setResult(data.getInt(0), intentResult);
